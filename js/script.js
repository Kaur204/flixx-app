const global=
{
  ...
  api: {
    apiKey: '2cea9cba8065edc69fd29b5ac1654bd9',
...
async function fetchAPIData(endpoint) {
    ...
    // only use this for development or very small projects. you should store your key and make request from a server
    const api_url= global.api.apiUrl+endpoint+"?api_key="+api_key+"&language=en-US";
    ...
    // const response = await fetch('https://api.themoviedb.org/3/movie/157336?api_key=1b777fa5bc1241c681e8ef3b3f33434b&append_to_response=videos,images');
...
async function searchAPIData(endpoint) {
  const api_key=global.api.apiKey;
  // only use this for development or very small projects. you should store your key and make request from a server
  const api_url= global.api.apiUrl+"search/"+global.search.type+"?api_key="+api_key+"&language=en-US&query="+ global.search.term+"&page="+global.search.page;
    ...
    link.forEach((item)=>
        {
            ...
            if(item.getAttribute('href')=== global.currPage)
            {
